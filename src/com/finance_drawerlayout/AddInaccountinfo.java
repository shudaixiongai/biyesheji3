...
public class AddInaccountinfo extends Activity {
	...
	public void clickSave(View v) {
		...
		if (!Smoney.isEmpty()) {// 判断金额不为空
			...
			if (!(Smoney.isEmpty() || time.isEmpty() || type.isEmpty()
					|| handler.isEmpty() || mark.isEmpty())) {
				try {
					InaccountManager inaccountManager = new InaccountManager(
							AddInaccountinfo.this);

					Tb_inaccount tb_inaccount = new Tb_inaccount(
							inaccountManager.getMaxId() + 1, money, time, type,
							handler, mark);

					inaccountManager.add(tb_inaccount);// 添加收入信息
